...
def setup_logger(logger):
    log_format = f'%(levelname)-8s| %(relativepath)s:%(lineno)s: %(message)s'
    formatter = logging.Formatter(log_format)

    # StreamHandler allows streaming of logs in console
    streamHandler = logging.StreamHandler()
    streamHandler.setFormatter(formatter)
    streamHandler.addFilter(PackagePathFilter())
    streamHandler.setLevel(logging.INFO)  # Logs only error level and above messages to stream
...
def get_headers():
    return {
        "Referer": "app.sli.do",
        "host": "app.sli.do",
        "x-client-id": '3xO6ZbDIXI3wl4o', # Just grabbed this from a request, you might need to change it but probably not
        "X-Slidoapp-Version": "SlidoParticipantApp/53.178.0 (web)",
        "user-agent": USER_AGENT_LIST[randint(0, len(USER_AGENT_LIST) - 1)]
    }


API = "https://app.sli.do/eu1/api/v0.5"
